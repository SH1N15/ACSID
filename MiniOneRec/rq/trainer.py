...
class Trainer(object):
    ...
    def __init__(self, args, model, data_num, fusion=None):
        self.args = args
        self.model = model
    ...
    def _prepare_input(self, batch):
        """Fuse a batch into the RQ-VAE input z_i.

        batch is a dict from FusedEmbDataset's default collate:
          z_text [B, D_text], z_cf [B, D_cf], alpha [B, 1].
        ...
        if isinstance(batch, dict):
            ...
            alpha = batch["alpha"].to(self.device).view(-1)
        else:
            z_text = batch.to(self.device)
            z_cf = None
            alpha = None
